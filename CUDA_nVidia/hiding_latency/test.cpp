...
extern "C"
void simple_copy_add_base(const real*, const real*, real*, unsigned int n);
extern "C"
void simple_copy_add_base_stride(const real*, const real*, real*, unsigned int n);
extern "C"
...
int main()
{
  ...
  unsigned int wf_count=WF_COUNT;
	std::cout << "wavefront count is " << wf_count << ", wavefront size is " << wf_size << '\n';
  size_t N = 1024 * 1024 * 64;
  std::vector<real> A(N);
  std::vector<real> B(N);
  std::vector<real> C(N);
  std::vector<real> Cref(N);
  std::vector<real> Cref_base(N);
  for(size_t i(0); i != N; i++)
  {
		A[i] = (real) 2.0 * (real) i;
    B[i] = - (real) i;
		register const real a = A[i];
		register const real b = B[i];
  ...
  if(!(dev_A && dev_B && dev_C))
	...
	#define TEST(test_func, ntimes, ref)         \
  ...
  pass &= check_resutls(C,ref, "\t");	   			 \
  std::cout << '\t' << #test_func << " bw is " << sizeof(real)*(A.size()+B.size()+C.size())/t.get_timing(#test_func)*1e6/1024.0/1024.0/1024.0 << " Gb/sec\n";
  
  TEST(simple_copy_add_base, 300, Cref_base)
  TEST(simple_copy_add_base_stride, 300, Cref_base)
  TEST(simple_copy_add, 300, Cref)
  TEST(simple_copy_add_stride, 300, Cref)
